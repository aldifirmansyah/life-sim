...
const L: Record<string, string[]> = {
  ...
  'greet.acquaintance': ['Eh Aldi! Good {time}.', 'Aldi! How are you?', 'Hello Aldi, you makan already?', 'Aldi ah, long time no see!'],
  'greet.friend': [
  ...
  'topic.neutral': ['{topic} ah? OK lah, not bad.', 'Hmm, {topic}. Can, can.', 'Oh, {topic}. I don\'t know much, but OK.'],
  'topic.dislike': [
  ...
  'ask.like': ['Me ah? I really like {topic}. Can talk the whole day.', 'You want to know? {topic}! Ask anyone, they will tell you.'],
  ...
  'compliment.good': ['Wah, thank you! You very sweet leh.', 'Aiyo, you make me paiseh. Thanks ah!', 'Really ah? Thank you, Aldi.'],
  ...
  'gossip.bad': ['{other} ah… don\'t get me started. Very lecheh one.', 'Between you and me, {other} and I, not so close.'],
...
export class TemplateProvider implements DialogueProvider {
  async getLine(ctx: DialogueContext): Promise<DialogueLine> {
    ...
    const emote = /good|loved|liked|friend/.test(key) ? 'happy' : /bad|dislike|cold|refuse/.test(key) ? 'annoyed' : 'neutral';
    return { text: text.charAt(0).toUpperCase() + text.slice(1), emote };
  }
}
